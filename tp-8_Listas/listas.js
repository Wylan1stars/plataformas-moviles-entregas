...
function calcularPromedio(listaDeNumeros) {
    const sumaTotal = listaDeNumeros.reduce((acumulador, valorActual) => acumulador + valorActual, 0);
    return sumaTotal / listaDeNumeros.length;
}
...
function enumerarLista(listaDePalabras) {
    ...
    const palabrasEnumeradas = listaDePalabras.slice(0, -1).join(", ") + " y " + listaDePalabras.slice(-1) + ".";
    return palabrasEnumeradas.charAt(0).toUpperCase() + palabrasEnumeradas.slice(1);
}
